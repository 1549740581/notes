...
- 选举出一个sentinel作为领导
- 选出一个slave作为master
- sentinel通知其它slave结点成为新的master的slave节点
- 通知客户端主从变化
- 等待宕机的master节点上线，并成为新的master的slave节点
...
实现一个master-两个slave主从复制模式，并且有三个sentinel节点对这三个节点进行监视。
...
- master、slave节点如果设置了密码应该一致（因为slave节点也有可能成为master节点），并且在sentinel配置文件中要进行配置：
...
# 原本sentinel-27000.conf配置文件也有变换，增加了一下内容
